...
const studies = [
  {
    slug: "securitypal-case", client: "SecurityPal", subject: "Positioning Film",
    industry: "Security", type: "Positioning Film",
    ...
    color: "#101418", year: "2024",
  },
  {
    slug: "securitypal-case", client: "SecurityPal", subject: "2025 Year in Review",
    industry: "Security", type: "Brand Film",
    ...
    color: "#0d1218", year: "2025",
  },
  {
    slug: "docunlock-case", client: "DocUnlock", subject: "Feature Launch",
    industry: "TradeOps", type: "Explainer",
    ...
    color: "#0d1018", year: "2024",
  },
  {
    slug: "aleph-case", client: "Aleph", subject: "Series B Announcement",
    industry: "Fintech", type: "Launch Film",
    ...
    <div style={{ background: "#0a0a0a", minHeight: "100vh", color: "#f2f2f2", fontFamily: "var(--font-bricolage), system-ui, sans-serif" }}>
      ...
      <nav style={{
        ...
        <Link href="/" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          {/* "Studio" — Bricolage */}
          <span style={{ fontFamily: "var(--font-bricolage)", fontSize: "11px", fontWeight: 300, color: "rgba(255,255,255,0.25)", letterSpacing: "0.02em" }}>Studio</span>
        </Link>

        <Link href="/"
          style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontFamily: "var(--font-bricolage)", fontSize: "12.5px", fontWeight: 300, color: "rgba(255,255,255,0.38)", textDecoration: "none", transition: "color 0.2s" }}
      ...
      <div style={{ padding: "100px 28px 140px", maxWidth: "1100px", margin: "0 auto" }}>
        ...
        <div ref={headerRef} style={{ marginBottom: "64px" }}>
          {/* EYEBROW — Bricolage */}
          <p className="h-el" style={{ opacity: 0, fontFamily: "var(--font-bricolage)", fontSize: "11px", letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(255,255,255,0.28)", fontWeight: 300, marginBottom: "20px" }}>
            Case Studies
          </p>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "24px", flexWrap: "wrap", marginBottom: "40px" }}>
            ...
            {/* SUBLINE — Bricolage */}
            <p className="h-el" style={{ opacity: 0, fontFamily: "var(--font-bricolage)", fontSize: "14px", fontWeight: 300, color: "rgba(255,255,255,0.28)", margin: 0, textAlign: "right", lineHeight: 1.7, maxWidth: "240px" }}>
            ...
            {ALL_FILTERS.map(f => (
              <button key={f} onClick={() => setActive(f)}
                style={{
                  ...
                  fontFamily: "var(--font-bricolage)", fontSize: "12.5px",
        ...
        {/* GRID */}
        <div ref={gridRef} style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: isMobile ? "12px" : "2px" }}>
          {filtered.map((s, i) => {
            ...
            return (
              <Link key={`${s.slug}-${i}`} href={`/case-studies/${s.slug}`} className="cs-card"
                style={{
                  opacity: 0, display: "block", textDecoration: "none", color: "inherit",
                  border: "1px solid rgba(255,255,255,0.07)", borderRadius: radius, overflow: "hidden",
                ...
                {/* THUMBNAIL */}
                <div style={{ width: "100%", aspectRatio: isMobile ? "16 / 9" : "16 / 8", background: s.color, position: "relative", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <img src={s.thumb} alt={s.client}
                    style={{ width: "100%", height: "100%", objectFit: "cover", opacity: isHov ? 0.82 : 0.62, transform: isHov ? "scale(1.03)" : "scale(1)", transition: "opacity 0.35s ease, transform 0.5s ease", display: "block" }}
                    onError={e => { const el = e.currentTarget as HTMLImageElement; if (el.src.includes("maxresdefault")) el.src = el.src.replace("maxresdefault", "hqdefault"); }} />

                  {/* TAGS OVERLAY — Bricolage */}
                  {!isMobile && (
                    <div style={{ position: "absolute", top: "14px", left: "14px", display: "flex", gap: "6px" }}>
                      {[s.industry, s.type].map(tag => (
                        <span key={tag} style={{ fontFamily: "var(--font-bricolage)", fontSize: "9.5px", fontWeight: 300, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.7)", background: "rgba(0,0,0,0.55)", backdropFilter: "blur(8px)", padding: "3px 8px", borderRadius: "4px", border: "1px solid rgba(255,255,255,0.1)" }}>{tag}</span>
                      ))}
                    </div>
                  )}

                  {/* YEAR — Bricolage */}
                  <span style={{ fontFamily: "var(--font-bricolage)", position: "absolute", top: "14px", right: "14px", fontSize: "10px", fontWeight: 300, color: "rgba(255,255,255,0.35)", background: "rgba(0,0,0,0.45)", backdropFilter: "blur(6px)", padding: "2px 7px", borderRadius: "4px" }}>{s.year}</span>

                  {/* PLAY ICON */}
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: isHov ? 1 : 0, transition: "opacity 0.25s" }}>
                    <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <svg width="13" height="15" viewBox="0 0 14 16" fill="none"><path d="M0 0L14 8 0 16z" fill="rgba(255,255,255,0.9)"/></svg>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: isMobile ? "20px 18px 22px" : "24px 28px 28px" }}>
                  ...
                  <p style={{ fontFamily: "var(--font-bricolage)", fontSize: "10px", fontWeight: 300, color: "rgba(255,255,255,0.22)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 10px" }}>
                    {s.client} — {s.subject}
                  </p>

                  {/* MOBILE TAGS — Bricolage */}
                  {isMobile && (
                    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "12px" }}>
                      {[s.industry, s.type].map(tag => (
                        <span key={tag} style={{ fontFamily: "var(--font-bricolage)", fontSize: "10px", fontWeight: 300, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.40)", border: "1px solid rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.03)", padding: "6px 10px", borderRadius: "999px" }}>{tag}</span>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", gap: "12px" }}>
                    {/* OUTCOME — Bricolage */}
                    <p style={{ fontFamily: "var(--font-bricolage)", fontSize: isMobile ? "13px" : "12px", fontWeight: 300, color: "rgba(255,255,255,0.25)", margin: 0, lineHeight: 1.55 }}>{s.outcome}</p>
                    {/* READ CTA — Bricolage */}
                    <span style={{ fontFamily: "var(--font-bricolage)", fontSize: "12px", fontWeight: 300, color: "rgba(255,255,255,0.42)", display: "flex", alignItems: "center", gap: "6px", whiteSpace: "nowrap", opacity: isMobile ? 1 : isHov ? 1 : 0, transition: "opacity 0.2s", marginTop: isMobile ? "2px" : 0 }}>
                      Read case study
        ...
        <div style={{ marginTop: "80px", paddingTop: "48px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "24px" }}>
          <div>
            ...
            <p style={{ fontFamily: "var(--font-dm)", fontSize: "18px", fontWeight: 700, color: "rgba(255,255,255,0.7)", letterSpacing: "-0.04em", margin: "0 0 6px" }}>Want results like these?</p>
            {/* CTA SUBLINE — Bricolage */}
            <p style={{ fontFamily: "var(--font-bricolage)", fontSize: "14px", fontWeight: 300, color: "rgba(255,255,255,0.28)", margin: 0 }}>Book a call. We'll scope the right format in 30 minutes.</p>
          </div>
