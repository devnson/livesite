...
export default function RecentWork() {
  ...
  // scroll → switch to Best
  useEffect(() => {
    const section = document.getElementById("recent-work");
    if (!section) return;
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && active !== "Best" && active !== "Spec" && active !== "Latest") {
        setActive("Best");
      }
    }, { threshold: 0.15 });
    obs.observe(section);
