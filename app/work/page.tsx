...
export default function WorkPage() {
  ...
  const [cols, setCols] = useState<number>(3); // responsive cols
    ...
    <div style={{ background: "#000", minHeight: "100vh", color: "#f2f2f2", fontFamily: "var(--font-dm), system-ui, sans-serif" }}>
      ...
      >
        <Link href="/" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.55)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
            Tanosei
          </span>
      ...
      <div style={{ padding: "140px 28px 140px", maxWidth: "1160px", margin: "0 auto" }}>
            ...
            {ALL_TYPES.map((type) => (
              <button
                ...
                style={{
                  padding: "7px 16px",
                  borderRadius: "999px",
                  border: active === type ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(255,255,255,0.08)",
        ...
        >
          {filtered.map((p, i) => {
            ...
            let radius = "0";
            // corner radii adjusted for dynamic cols
            ...
            else if (cols === 1 && i === filtered.length - 1) radius = "0 0 14px 14px"; // single column last card

            // responsive paddings / typography
            const cardPadding = cols === 1 ? "18px 18px 22px" : "20px 22px 24px";
            const titleSize = cols === 1 ? "16px" : "15px";
            const deliverableSize = cols === 1 ? "13px" : "12px";
            const showTagsOverlay = cols > 1; // hide small tags overlay on narrow mobile
            ...
            return (
              ...
              >
                ...
                >
                  {p.gif ? (
                    <img src={p.gif} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", opacity: isHovered ? 0.85 : 0.6, transition: "opacity 0.3s" }} />
                  ) : p.videoId ? (
                    <img src={`https://img.youtube.com/vi/${p.videoId}/hqdefault.jpg`} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", opacity: isHovered ? 0.75 : 0.55, transition: "opacity 0.3s" }} />
                  ) : p.thumb ? (
                    <img src={p.thumb} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", transform: isHovered ? "scale(1.03)" : "scale(1)", transition: "transform 0.5s ease" }} />
                  ) : (
                    <>
                      ...
                      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <span style={{ fontFamily: "var(--font-dm)", fontWeight: 800, fontSize: "clamp(20px, 3vw, 36px)", color: "rgba(255,255,255,0.06)", letterSpacing: "-0.04em" }}>
                          {p.client}
                        </span>
                      </div>
                    </>
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: isHovered ? 1 : 0, transition: "opacity 0.22s" }}>
                    <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  {/* Tags overlay (hidden on single column) */}
                  {showTagsOverlay && (
                ...
                <div style={{ padding: cardPadding }}>
                  ...
                  <h3 style={{ fontFamily: "var(--font-dm)", fontWeight: 700, fontSize: titleSize, letterSpacing: "-0.02em", margin: "6px 0 4px", color: isHovered ? "rgba(255,255,255,0.82)" : "rgba(255,255,255,0.7)", transition: "color 0.2s" }}>
                    {p.client} — {p.title}
                  </h3>
                  <p style={{ fontSize: deliverableSize, color: "rgba(255,255,255,0.28)", margin: 0, lineHeight: 1.55 }}>{p.deliverable}</p>
                </div>
              </div>
            );
          })}
          ...
          <div ref={modalRef} onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "620px", background: "#111", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", overflow: "hidden", opacity: 0 }}>
            ...
            <div style={{ aspectRatio: "16/9", position: "relative", background: "#000" }}>
              {modal.videoId ? (
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "14px", background: "linear-gradient(135deg, #0d0d0d, #111)" }}>
                  <div style={{ width: "52px", height: "52px", borderRadius: "50%", border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ padding: "28px 28px 24px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
                <Link href={modal.caseUrl} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "10px 22px", borderRadius: "8px", background: "#fff", color: "#000", fontSize: "13px", fontWeight: 700, textDecoration: "none", fontFamily: "var(--font-dm)", transition: "opacity 0.2s" }} onMouseEnter={(e) => ((e.currentTarget as HTMLAnchorElement).style.opacity = "0.88")} onMouseLeave={(e) => ((e.currentTarget as HTMLAnchorElement).style.opacity = "1")}>
                  View full case study →
                </Link>

                <button onClick={() => setModal(null)} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.4)", borderRadius: "8px", padding: "10px 20px", fontSize: "13px", cursor: "pointer", fontFamily: "var(--font-dm)", transition: "border-color 0.2s, color 0.2s" }} onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(255,255,255,0.25)"; (e.currentTarget as HTMLButtonElement).style.color = "rgba(255,255,255,0.7)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(255,255,255,0.1)"; (e.currentTarget as HTMLButtonElement).style.color = "rgba(255,255,255,0.4)"; }}>
                  Close
                </button>
              </div>
            </div>
      ...
      <script dangerouslySetInnerHTML={{ __html: `(function(){ var el = document.getElementById('cursorLightWork'); if(!el) return; window.addEventListener('mousemove', function(e){ el.style.left = e.clientX + 'px'; el.style.top = e.clientY + 'px'; }, { passive: true }); })();` }} />
    </div>
  );
}