...
export default function HowWeWork() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.shiftKey && e.key === "P") {
        setPresMode(prev => {
          const next = !prev;
          if (tlRef.current) { next ? tlRef.current.pause() : tlRef.current.play(); setIsPlaying(!next); }
          return next;
        });
      }
    };
    ...
    const ctx = gsap.context(() => {
      ...
      const glowCard = (col: number, side: 0 | 1, color: string) => {
        ...
        tl.to(el, { borderColor: `${color}AA`, boxShadow: `0 0 18px 2px ${color}33, inset 0 0 14px ${color}22`, duration: 0.28, ease: "power2.out" }, ">")
          .to(el, { boxShadow: "none", borderColor: (el as any).dataset.role === "client" ? "rgba(96,165,250,0.18)" : "rgba(255,255,255,0.10)", duration: 0.55, ease: "power2.out" }, ">+=0.35");
      };
      ...
      if (finalCard) {
        tl.to(finalCard, { background:"rgba(255,255,255,0.10)", borderColor:"rgba(255,255,255,0.55)", boxShadow:"0 0 40px 8px rgba(255,255,255,0.12), inset 0 0 30px rgba(255,255,255,0.08)", duration:0.5, ease:"power2.out" }, ">")
          ...
          .to(finalCard, { background:"rgba(255,255,255,0.03)", borderColor:"rgba(255,255,255,0.10)", boxShadow:"none", duration:1.2, ease:"power2.out" }, ">+=0.4");
      } else {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const glowMobileCard = (i: number, color: string) => {
        ...
        tl.to(el, { borderColor: `${color}AA`, boxShadow: `0 0 18px 2px ${color}33, inset 0 0 14px ${color}22`, duration: 0.28, ease: "power2.out" }, ">")
          .to(el, { boxShadow: "none", borderColor: isClient ? "rgba(96,165,250,0.18)" : "rgba(255,255,255,0.10)", duration: 0.55, ease: "power2.out" }, ">+=0.35");
      };
        ...
        if (i === mobileSteps.length - 1) {
          ...
          if (finalEl) {
            tl.to(finalEl, { background: "rgba(255,255,255,0.10)", borderColor: "rgba(255,255,255,0.55)", boxShadow: "0 0 40px 8px rgba(255,255,255,0.12), inset 0 0 30px rgba(255,255,255,0.08)", duration: 0.5, ease: "power2.out" }, ">")
              ...
              .to(finalEl, { background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.10)", boxShadow: "none", duration: 1.2, ease: "power2.out" }, ">+=0.4");
          }
        }
      });
  ...
  if (isMobile) {
    return (
      <section id="how-we-work" ref={mobileContainerRef}
        style={{ padding: "80px 0 100px", background: "#080808", borderTop: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
  ...
  return (
    <section id="how-we-work" ref={containerRef}
      style={{ padding: "120px 28px 140px", background: "#080808", borderTop: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
      <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${COL}, 1fr)`, gap: "0 12px", height: `${CONN_H}px` }}>
          {columns.map((col, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "center" }}>
              {col.top && <div ref={el => { connRefs.current[i][0] = el; }} style={{ width: "1px", height: "100%", background: "rgba(255,255,255,0.12)" }} />}
            </div>
          ))}
        </div>
        ...
        <div ref={spineWrapRef} style={{ position: "relative", height: "2px", zIndex: 1 }}>
          <div style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,0.09)", zIndex: 1 }} />
          <div style={{ position: "absolute", top: "50%", left: 0, right: 0, transform: "translateY(-50%)", display: "grid", gridTemplateColumns: `repeat(${COL}, 1fr)`, gap: "0 12px", zIndex: 5, pointerEvents: "none" }}>
            {columns.map((_, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <div ref={el => { dotRefs.current[i] = el; }} style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#080808", border: "1px solid rgba(255,255,255,0.32)", boxShadow: "0 0 10px rgba(255,255,255,0.16)" }} />
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${COL}, 1fr)`, gap: "0 12px", height: `${CONN_H}px` }}>
          {columns.map((col, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "center" }}>
              {col.bottom && <div ref={el => { connRefs.current[i][1] = el; }} style={{ width: "1px", height: "100%", background: "rgba(255,255,255,0.12)" }} />}
            </div>
          ))}
        </div>
      ...
      {presMode && (
        <div style={{ position: "fixed", bottom: "28px", left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: "8px", background: "rgba(8,8,8,0.95)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "999px", padding: "8px 14px", backdropFilter: "blur(16px)", boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 9999, userSelect: "none" }}>
          <button onClick={() => goToStep(stepIdx - 1)} disabled={stepIdx === 0} style={{ background: "none", border: "none", cursor: stepIdx === 0 ? "default" : "pointer", padding: "4px 8px", borderRadius: "6px", color: stepIdx === 0 ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.75)", fontSize: "14px", lineHeight: 1 }}>‹</button>
          <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            {STEP_LABELS.map((label, i) => (
              <button key={i} onClick={() => goToStep(i)} title={label} style={{ width: i === stepIdx ? "20px" : "6px", height: "6px", borderRadius: "999px", border: "none", cursor: "pointer", padding: 0, background: i === stepIdx ? "#60a5fa" : "rgba(255,255,255,0.22)", transition: "all 0.25s ease" }} />
            ))}
          </div>
