...
export default function Nav() {
  ...
  const NAV_OFFSET = 96; // matches your pill + padding
  ...
  const handleNav = useCallback(
    (id: string) => {
      ...
      // If you're already on home, just scroll.
      if (pathname === "/") {
        scrollToId(id);
        return;
      }

      // If you're on another route, go to home with hash.
      // Then after route change, do a real scroll (hash jump is unreliable).
      router.push(`/#${id}`);

      // wait a tick for the page to render, then scroll
      setTimeout(() => scrollToId(id), 50);
      setTimeout(() => scrollToId(id), 250); // second attempt for slower loads
    },
    [pathname, router, scrollToId]
  );

  // If user loads directly on /#work etc, make sure we scroll correctly
  useEffect(() => {
    if (pathname !== "/") return;
    const hash = window.location.hash?.replace("#", "");
    if (!hash) return;

    // allow layout/components to paint
      ...
      >
        ...
        {!isMobile && (
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            >
              <span style={{ padding: "10px 12px 10px 20px" }}>
                Book a Clarity Call
              </span>
              <span
                style={{
              ...
              >
                <svg
                  ...
                  strokeWidth="2.5"
                  style={{
                    transform: btnHovered ? "rotate(0deg)" : "rotate(-45deg)",
