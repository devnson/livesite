...
const creative: Member = { name: "Creative Lead", role: "", initials: "S", photo: "/illustrations/sushan.png" };
...
function buildSegs(pts: [number, number][]) {
  const segs: Seg[] = []; let total = 0;
  for (let i = 0; i < pts.length - 1; i++) {
    const dx = pts[i+1][0]-pts[i][0], dy = pts[i+1][1]-pts[i][1];
    const len = Math.sqrt(dx*dx+dy*dy);
    segs.push({ x1:pts[i][0], y1:pts[i][1], x2:pts[i+1][0], y2:pts[i+1][1], len }); total += len;
  }
  return { segs, total };
}
function posAt(segs: Seg[], total: number, d: number) {
  ...
  for (const s of segs) {
    if (rem <= s.len+0.001) { const t = s.len>0?rem/s.len:0; return { x:s.x1+(s.x2-s.x1)*t, y:s.y1+(s.y2-s.y1)*t }; }
    rem -= s.len;
  }
  const l = segs[segs.length-1]; return { x:l.x2, y:l.y2 };
}

const SPEED=150, SPAWN_INTERVAL=0.38, FADE_IN=22, FADE_OUT=28;
...
type LayoutDesktop = { mode:"desktop"; W:number; H:number; COL_STUDIO:number; COL_CREATIVE:number; COL_OPS:number; COL_TEAM:number; spineX:number; STUDIO_Y:number; CREATIVE_Y:number; OPS_Y:number; memberY:(i:number)=>number; studioMY:number; creativeMY:number; opsMY:number; topMY:number; botMY:number; TRUNK:ReturnType<typeof buildSegs>; SPINE_UP:ReturnType<typeof buildSegs>; SPINE_DOWN:ReturnType<typeof buildSegs>; BRANCHES:ReturnType<typeof buildSegs>[]; };
type LayoutMobile  = { mode:"mobile"; W:number; H:number; centerX:number; STUDIO_Y:number; CREATIVE_Y:number; OPS_Y:number; studioMY:number; creativeMY:number; opsMY:number; gridLeftX:number; gridRightX:number; gridRow1Y:number; gridRow2Y:number; lanes:{x:number;y:number}[]; spineX:number; spineBotY:number; TRUNK:ReturnType<typeof buildSegs>; SPINE_DOWN:ReturnType<typeof buildSegs>; BRANCHES:ReturnType<typeof buildSegs>[]; };
type Layout = LayoutDesktop | LayoutMobile;

function Avatar({ m, isHover }: { m: Member; isHover: boolean }) {
  const r=22, cx=CW/2, cy=40;
  const tune=avatarTuning[m.name]||{};
  const imgSize=((tune.size??165)/100)*(r*2);
  const px=(tune.pos?.[0]??50)/100, py=(tune.pos?.[1]??50)/100;
  const imgX=Math.round(cx-r-(imgSize-r*2)*px), imgY=Math.round(cy-r-(imgSize-r*2)*py);
  ...
  return (
    <>
      ...
      {m.photo ? (
        ...
        <text x={cx} y={cy+6} textAnchor="middle" fontFamily="var(--font-bricolage)" fontSize="13" fontWeight="500" fill="rgba(255,255,255,0.55)">{m.initials||""}</text>
      )}
...
export default function Team() {
  ...
  const layout: Layout = useMemo(() => {
    if (!isMobile) {
      ...
      const H=TEAM_TOTAL_H+TOP_PAD*2, VCENTER=H/2;
      const COL_STUDIO=0, COL_CREATIVE=COL_STUDIO+CW+HGAP, COL_OPS=COL_CREATIVE+CW+HGAP, COL_TEAM=COL_OPS+CW+HGAP;
      const spineX=COL_OPS+CW+HGAP/2;
      const STUDIO_Y=VCENTER-CH/2, CREATIVE_Y=VCENTER-CH/2, OPS_Y=VCENTER-CH/2;
      const memberY=(i:number)=>TOP_PAD+i*(CH+VGAP);
      const studioMY=STUDIO_Y+CH/2, creativeMY=CREATIVE_Y+CH/2, opsMY=OPS_Y+CH/2;
      const topMY=memberY(0)+CH/2, botMY=memberY(teamMembers.length-1)+CH/2;
    ...
    const STUDIO_Y=TOP_PAD, CREATIVE_Y=STUDIO_Y+CH+VSTEP, OPS_Y=CREATIVE_Y+CH+VSTEP;
    const studioMY=STUDIO_Y+CH/2, creativeMY=CREATIVE_Y+CH/2, opsMY=OPS_Y+CH/2;
    const GRID_GAP_X=22, GRID_GAP_Y=22;
    const gridTopY=OPS_Y+CH+84, gridRow1Y=gridTopY, gridRow2Y=gridTopY+CH+GRID_GAP_Y;
    const gridW=CW*2+GRID_GAP_X, W=Math.max(gridW+40,CW+40);
    const centerX=W/2, gridLeftX=centerX-gridW/2, gridRightX=gridLeftX+CW+GRID_GAP_X;
    const lanes=[{ x:gridLeftX+CW/2,y:gridRow1Y+CH/2 },{ x:gridRightX+CW/2,y:gridRow1Y+CH/2 },{ x:gridLeftX+CW/2,y:gridRow2Y+CH/2 },{ x:gridRightX+CW/2,y:gridRow2Y+CH/2 }];
    const spineX=centerX, spineBotY=(lanes[0].y+lanes[2].y)/2, H=gridRow2Y+CH+18;
  ...
  useEffect(() => {
    ...
    spineBlocks.current=[]; branchBlocks.current=[];
    lastTsRef.current=null; spawnTimerRef.current=0;
  }, [layout.mode]);
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      const dt=Math.min((ts-lastTsRef.current)/1000,0.05); lastTsRef.current=ts;
      const move=SPEED*dt;
      spawnTimerRef.current+=dt;
      while (spawnTimerRef.current>=SPAWN_INTERVAL) { spawnTimerRef.current-=SPAWN_INTERVAL; trunkBlocks.current.push({ id:uid(), dist:0, spawnedSpine:false }); }
      trunkBlocks.current=trunkBlocks.current.reduce<TrunkBlock[]>((acc,b)=>{
        const next=b.dist+move, trunkTotal=layout.TRUNK.total;
        if (!b.spawnedSpine && next>=trunkTotal) {
          const emitted=new Array(teamMembers.length).fill(false);
          if (layout.mode==="desktop") { spineBlocks.current.push({ id:uid(), dist:0, emitted:[...emitted] }); spineBlocks.current.push({ id:uid(), dist:0, emitted:[...emitted] }); }
          else spineBlocks.current.push({ id:uid(), dist:0, emitted:[...emitted] });
        }
      ...
      if (layout.mode==="desktop") {
        spineBlocks.current=spineBlocks.current.reduce<SpineBlock[]>((acc,s,idx)=>{
          const isUp=idx%2===0, path=isUp?layout.SPINE_UP:layout.SPINE_DOWN, next=s.dist+move;
          const p=posAt(path.segs,path.total,next);
          for (let i=0;i<teamMembers.length;i++) {
            if (s.emitted[i]) continue;
            const laneY=layout.memberY(i)+CH/2, isLaneAbove=laneY<=layout.opsMY, laneMatchesDir=isUp?isLaneAbove:!isLaneAbove;
            if (!laneMatchesDir) continue;
            if (Math.abs(p.y-laneY)<10) { s.emitted[i]=true; branchBlocks.current.push({ id:uid(), lane:i, dist:0 }); }
          }
          if (next<=path.total+30) acc.push({ ...s, dist:next }); return acc;
        }, []);
      } else {
        spineBlocks.current=spineBlocks.current.reduce<SpineBlock[]>((acc,s)=>{
          const path=layout.SPINE_DOWN, next=s.dist+move, p=posAt(path.segs,path.total,next);
          for (let i=0;i<teamMembers.length;i++) { if (s.emitted[i]) continue; if (Math.abs(p.y-layout.lanes[i].y)<10) { s.emitted[i]=true; branchBlocks.current.push({ id:uid(), lane:i, dist:0 }); } }
          if (next<=path.total+30) acc.push({ ...s, dist:next }); return acc;
        }, []);
      }
      branchBlocks.current=branchBlocks.current.reduce<BranchBlock[]>((acc,b)=>{
        const br=layout.BRANCHES[b.lane], next=b.dist+move;
        if (next<=br.total+25) acc.push({ ...b, dist:next }); return acc;
      }, []);
      redraw(n=>n+1); raf=requestAnimationFrame(loop);
    };
  ...
  if (layout.mode==="desktop") {
    for (let idx=0;idx<spineBlocks.current.length;idx++) {
      const s=spineBlocks.current[idx], isUp=idx%2===0, path=isUp?layout.SPINE_UP:layout.SPINE_DOWN;
    ...
    for (const s of spineBlocks.current) {
      const path=layout.SPINE_DOWN, p=posAt(path.segs,path.total,s.dist);
  ...
  for (const b of branchBlocks.current) {
    const br=layout.BRANCHES[b.lane], p=posAt(br.segs,br.total,b.dist);
    blocks.push(renderBlock(`b-${b.id}`,p.x,p.y,Math.min(b.dist/FADE_IN,1)*Math.min((br.total-b.dist)/FADE_OUT,1)));
  }

  const cardFill="rgb(12,12,12)", strokeIdle="rgba(255,255,255,0.10)", strokeHover="rgba(255,255,255,0.22)";
  const card=(x:number,y:number,m:Member,isHover:boolean,rx=18)=>(
    <g key={m.name} transform={`translate(${x}, ${y})`} onMouseEnter={()=>setHov(m.name)} onMouseLeave={()=>setHov(null)} style={{ cursor:"default" }}>
      ...
      {/* NAME — DM Sans */}
      <text x={CW/2} y={83} textAnchor="middle" fontFamily="var(--font-dm)" fontSize="12" fontWeight="700" letterSpacing="-0.03em" fill={isHover?"rgba(255,255,255,0.78)":"rgba(255,255,255,0.50)"}>{m.name}</text>
      {/* ROLE — Bricolage */}
      <text x={CW/2} y={100} textAnchor="middle" fontFamily="var(--font-bricolage)" fontSize="9.5" fontWeight="300" fill="rgba(255,255,255,0.22)">{m.role}</text>
    </g>
  );
    ...
    <section id="team" style={{ padding:"120px 24px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ maxWidth:"1160px", margin:"0 auto" }}>
        ...
        <div ref={headerRef} style={{ marginBottom:"72px", opacity:0 }}>
          {/* HEADING — DM Sans */}
          <h2 style={{ fontFamily:"var(--font-dm)", fontWeight:800, fontSize:"clamp(32px, 5vw, 56px)", letterSpacing:"-0.04em", lineHeight:1.05, margin:0, color:"rgba(255,255,255,0.92)" }}>
        ...
        <div ref={wrapRef} style={{ display:"flex", justifyContent:"center", overflowX:"auto", opacity:0, paddingBottom:8 }}>
          <svg viewBox={`0 0 ${layout.W} ${layout.H}`} style={{ width:isMobile?"min(100%, 320px)":"min(100%, 980px)", height:"auto", overflow:"visible", display:"block" }}>
            {layout.mode==="desktop" ? (
              <>
        ...
        <div style={{ display:"flex", justifyContent:"center", marginTop:"64px" }}>
          {/* BUTTON — Bricolage */}
          <a href="/team" style={{ display:"inline-flex", alignItems:"center", gap:"10px", padding:"14px 28px", borderRadius:"12px", border:"1px solid rgba(255,255,255,0.1)", background:"rgba(255,255,255,0.02)", color:"rgba(255,255,255,0.55)", fontFamily:"var(--font-bricolage)", fontSize:"13px", fontWeight:500, textDecoration:"none", transition:"all 0.22s", letterSpacing:"0.01em" }}
